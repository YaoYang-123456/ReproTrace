...
refresh. A failed attempt after invalidation may leave both absent, while a
report publication failure may leave fresh verification with no report. A
report without current canonical verification is historical, and file existence
alone does not prove that the most recent invocation succeeded. Read-only
`verify_bundle(write=False)` never modifies either output.
